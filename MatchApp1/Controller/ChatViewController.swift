...
class ChatViewController: MessagesViewController,MessagesDataSource,MessagesLayoutDelegate,MessagesDisplayDelegate,InputBarAccessoryViewDelegate,MessageCellDelegate,UIImagePickerControllerDelegate,UINavigationBarDelegate, UINavigationControllerDelegate,GetAttachProtocol {
...
//            cameraPicker.showsCameraControls = true
    ...
    func  loadMessage(){
        ...
        db.collection("Users").document(Auth.auth().currentUser!.uid).collection("matching").document(userDataModel!.uid!).collection("chat").order(by: "date").addSnapshotListener { [self] snapShot, error in
            ...
            if let snapShotDoc = snapShot?.documents{
                ...
                for doc in snapShotDoc{
                    ...
                    if let senderID = data["senderID"] as? String,let profileImageString = data["imageURLString"] as? String,let date = data["date"] as? TimeInterval,let _ = data["attachImageString"] as? String{
                        
                        //senderがどちらが送ったを検証。idで分けて自分と相手のメッセージを2つ作る
    ...
    func backgroundColor(for message: MessageType, at indexPath: IndexPath, in messagesCollectionView: MessagesCollectionView) -> UIColor {
        ...
        return isFromCurrentSender(message: message) ? Util.serChatColor(jibun: true) : Util.serChatColor(jibun: false)
    }
